            ...
            if (!cont) {
                ...
                if (fs.existsSync(filePath)) {
                    ...
                    if (index != null && index != '' && index != undefined) {
                        let add = canAdd(savedData_json, tipo, clave, fecha, correlativo) // Verificar si el comp no esta en el JSON
                        let tempData = []
                        ...
                        if (canAdd(savedData_json, tipo, clave, fecha, correlativo)){
                            const tempComp = {
                                tipo,
                                clave,
                                fecha: formatedDate,
                    ...
                    if (added)
                        ...
                        res.status(400).send({ msg: "El Comprobante ya está en la Caja" })
                } else {
                    res.status(400).send({ msg: "No existe el guardado temporal"})
                }
            } else {
                res.status(400).send({ msg: "El comprobante YA EXISTE EN LA CAJA: " + cont.caja })
            }
        }
    } catch (err) {
        res.status(500).send({ msg: "Ha ocurrido un error: " + err })
...
function removeOne(req, res) {
    ...
    console.log(savedData_json.length);
...
async function deleteAllCont(req, res) {

    const codigo = req.params.codigo

    try {
        const removed = await Contenido.deleteMany({ "caja": codigo })
        if (!removed) {
            res.status(400).send({ msg: "Error, no se encontró la caja." })
        } else {
            if (fs.existsSync(tmpRoot + codigo + ".txt")) {// al vaciar la caja también vaciamos su txt temporal
                fs.writeFile(tmpRoot + codigo + ".txt", "", (err) => {
                    if (err) {
                        console.log('Ha ocurrido un error al borra el almacenamiento temporal: ' + err)
                    }
                })
            }
            res.status(200).send({ msg: "Se ha borrado todo el contenido de la caja: " + codigo })
        }
    } catch (error) {
        res.status(500).send({ msg: "Ha ocurrido un error al eliminar el contenido de la caja" })
    }
}
